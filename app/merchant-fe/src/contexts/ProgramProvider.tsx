...
import { CampaignId, CampaignConfig, ProgramContext, Campaign, ProgramContextState, CampaignState } from "../hooks/useProgram";
...
export const ProgramProvider: FC<ProgramProviderProps> = ({ children }) => {
  ...
  return (
    <ProgramContext.Provider value={client} >
      {children}
...
class Client implements ProgramContextState {
  ...
  static toCampaignState(a: any): CampaignState {
    if (a.initialized) { return CampaignState.Initialized }
    if (a.started) { return CampaignState.Started }
    if (a.stopped) { return CampaignState.Stopped }
    if (a.revoked) { return CampaignState.Revoked }
