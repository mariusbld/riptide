...
export const PaymentProvider: FC<PaymentProviderProps> = ({ children }) => {
    ...
    const { recipient, reference: addlReference, splToken, label, requiredConfirmations, connectWallet } = useConfig();
    ...
    const referenceValue = useMemo<PublicKey | PublicKey[] | undefined>(() => {
        if (!reference) {
            return undefined;
        }
        if (!addlReference.length) {
            return reference;
        }
        return [reference, ...addlReference];
    }, [reference, addlReference]);
    ...
    const url = useMemo(
        ...
        [recipient, amount, splToken, referenceValue, label, message, memo]
    );
