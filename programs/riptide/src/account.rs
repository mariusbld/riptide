...
impl Campaign {
  ...
  pub fn add_vault(&mut self, vault: Vault) -> ProgramResult {
    require!(self.state == CampaignState::Initialized, CampaignError::InvalidState);
    require!(self.vaults.len() == 0, CampaignError::VaultAlreadyInitialized);
    self.vaults.push(vault);
    Ok(())
  }
  pub fn remove_vault(&mut self, vault: Vault) -> ProgramResult {
    require!(self.state == CampaignState::Revoked, CampaignError::InvalidState);
